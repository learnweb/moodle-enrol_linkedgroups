...
global $CFG;
require_once($CFG->dirroot . '/enrol/linkedgroups/lib.php');
...
class linkedgroups_test extends \advanced_testcase {
    ...
    protected function setUp(): void {
        global $CFG;
        $this->resetAfterTest();
        $CFG->enrol_plugins_enabled .= ',linkedgroups';
    }

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('linkedgroups'));
        $plugin = enrol_get_plugin('linkedgroups');
        $this->assertInstanceOf('enrol_linkedgroups_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_linkedgroups', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_linkedgroups', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $linkedgroupsplugin->sync($trace, null);
        $linkedgroupsplugin->sync($trace, $SITE->id);
    }

    public function test_longtimnosee() {
        global $DB;

        $linkedgroups = enrol_get_plugin('linkedgroups');
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol' => 'linkedgroups')));
        $instance1 = $DB->get_record('enrol', array('courseid' => $course1->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid' => $course2->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid' => $course3->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $id = $linkedgroups->add_instance($course3, array('status' => ENROL_INSTANCE_ENABLED, 'roleid' => $teacherrole->id));
        ...
        $linkedgroups->enrol_user($instance1, $user1->id, $studentrole->id);
        $linkedgroups->enrol_user($instance1, $user2->id, $studentrole->id);
        $linkedgroups->enrol_user($instance1, $user3->id, $studentrole->id);
        $this->assertEquals(3, $DB->count_records('user_enrolments'));
        $DB->insert_record('user_lastaccess',
                array('userid' => $user2->id, 'courseid' => $course1->id, 'timeaccess' => $now - 60 * 60 * 24 * 20));
        $DB->insert_record('user_lastaccess',
                array('userid' => $user3->id, 'courseid' => $course1->id, 'timeaccess' => $now - 60 * 60 * 24 * 2));
        ...
        $linkedgroups->enrol_user($instance3, $user1->id, $studentrole->id);
        $linkedgroups->enrol_user($instance3, $user2->id, $studentrole->id);
        $linkedgroups->enrol_user($instance3, $user3->id, $studentrole->id);
        $linkedgroups->enrol_user($instance3b, $user1->id, $teacherrole->id);
        $linkedgroups->enrol_user($instance3b, $user4->id, $teacherrole->id);
        $this->assertEquals(8, $DB->count_records('user_enrolments'));
        $DB->insert_record('user_lastaccess',
                array('userid' => $user2->id, 'courseid' => $course3->id, 'timeaccess' => $now - 60 * 60 * 24 * 11));
        $DB->insert_record('user_lastaccess',
                array('userid' => $user3->id, 'courseid' => $course3->id, 'timeaccess' => $now - 60 * 60 * 24 * 200));
        $DB->insert_record('user_lastaccess',
                array('userid' => $user4->id, 'courseid' => $course3->id, 'timeaccess' => $now - 60 * 60 * 24 * 200));
        ...
        $linkedgroups->sync($trace, $course2->id);
        $output = $trace->get_buffer();
        $trace->reset_buffer();
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertStringContainsString('No expired enrol_linkedgroups enrolments detected', $output);
    ...
    public function test_expired() {
        ...
        $instance3 = $DB->get_record('enrol', array('courseid' => $course3->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $this->assertEquals($studentrole->id, $instance3->roleid);
        $id = $linkedgroupsplugin->add_instance($course3, array('status' => ENROL_INSTANCE_ENABLED, 'roleid' => $teacherrole->id));
        $instance3b = $DB->get_record('enrol', array('id' => $id), '*', MUST_EXIST);
        ...
        $linkedgroupsplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $linkedgroupsplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $linkedgroupsplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now - 60);

        $linkedgroupsplugin->enrol_user($instance3, $user1->id, $studentrole->id, 0, 0);
        $linkedgroupsplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now - 60 * 60);
        $linkedgroupsplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now + 60 * 60);
        $linkedgroupsplugin->enrol_user($instance3b, $user1->id, $teacherrole->id, $now - 60 * 60 * 24 * 7, $now - 60);
        $linkedgroupsplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, $linkedgroupsplugin->get_config('expiredaction'));
        $linkedgroupsplugin->sync($trace, null);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));

        $linkedgroupsplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $linkedgroupsplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        ...
        $linkedgroupsplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_send_expiry_notifications() {
        ...
        $linkedgroupsplugin->set_config('expirynotifylast', $now - 60 * 60 * 24);
        $linkedgroupsplugin->set_config('expirynotifyhour', 0);
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol' => 'linkedgroups')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid' => $course4->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        ...
        $linkedgroupsplugin->enrol_user($instance1, $user1->id, $studentrole->id, 0, $now + 60 * 60 * 24 * 1,
                ENROL_USER_SUSPENDED); // Suspended users are not notified.
        $linkedgroupsplugin->enrol_user($instance1, $user2->id, $studentrole->id, 0,
                $now + 60 * 60 * 24 * 5);                       // Above threshold are not notified.
        $linkedgroupsplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0,
                $now + 60 * 60 * 24 * 3 + 60 * 60);               // Less than one day after threshold - should be notified.
        $linkedgroupsplugin->enrol_user($instance1, $user4->id, $studentrole->id, 0,
                $now + 60 * 60 * 24 * 4 - 60 * 3);                // Less than one day after threshold - should be notified.
        $linkedgroupsplugin->enrol_user($instance1, $user5->id, $studentrole->id, 0,
                $now + 60 * 60);                            // Should have been already notified.
        $linkedgroupsplugin->enrol_user($instance1, $user6->id, $studentrole->id, 0,
                $now - 60);                               // Already expired.
        $manualplugin->enrol_user($maninstance1, $user7->id, $editingteacherrole->id);
        $manualplugin->enrol_user($maninstance1, $user8->id,
                $managerrole->id);                                        // Highest role --> enroller.

        $linkedgroupsplugin->enrol_user($instance2, $user1->id, $studentrole->id);
        $linkedgroupsplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0,
                $now + 60 * 60 * 24 * 1 + 60 * 3);                // Above threshold are not notified.
        $linkedgroupsplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0,
                $now + 60 * 60 * 24 * 1 - 60 * 60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance3, $user1->id, $editingteacherrole->id);
        $linkedgroupsplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0,
                $now + 60 * 60 * 24 * 1 + 60);                  // Above threshold are not notified.
        $linkedgroupsplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0,
                $now + 60 * 60 * 24 * 1 - 60 * 60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance4, $user4->id, $editingteacherrole->id);
        $linkedgroupsplugin->enrol_user($instance4, $user5->id, $studentrole->id, 0, $now + 60 * 60 * 24 * 1 + 60);
        $linkedgroupsplugin->enrol_user($instance4, $user6->id, $studentrole->id, 0, $now + 60 * 60 * 24 * 1 - 60 * 60);
    ...
    public function test_show_enrolme_link() {
        ...
        $DB->update_record('enrol', $instance1);
        $linkedgroupsplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);

        // New enrolments are not allowed, but enrolment instance is enabled.
        $instance2 = $DB->get_record('enrol', array('courseid' => $course2->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance2->customint6 = 0;
        $DB->update_record('enrol', $instance2);
        $linkedgroupsplugin->update_status($instance2, ENROL_INSTANCE_ENABLED);

        // New enrolments are allowed , but enrolment instance is disabled.
        $instance3 = $DB->get_record('enrol', array('courseid' => $course3->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance3->customint6 = 1;
        $DB->update_record('enrol', $instance3);
        $linkedgroupsplugin->update_status($instance3, ENROL_INSTANCE_DISABLED);

        // New enrolments are not allowed and enrolment instance is disabled.
        $instance4 = $DB->get_record('enrol', array('courseid' => $course4->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance4->customint6 = 0;
        $DB->update_record('enrol', $instance4);
        $linkedgroupsplugin->update_status($instance4, ENROL_INSTANCE_DISABLED);

        // Cohort member test.
        $instance5 = $DB->get_record('enrol', array('courseid' => $course5->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance5->customint6 = 1;
        $instance5->customint5 = $cohort1->id;
        $DB->update_record('enrol', $instance1);
        $linkedgroupsplugin->update_status($instance5, ENROL_INSTANCE_ENABLED);

        $id = $linkedgroupsplugin->add_instance($course5, $linkedgroupsplugin->get_instance_defaults());
        $instance6 = $DB->get_record('enrol', array('id' => $id), '*', MUST_EXIST);
        $instance6->customint6 = 1;
        $instance6->customint5 = $cohort2->id;
        $DB->update_record('enrol', $instance1);
        $linkedgroupsplugin->update_status($instance6, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in future.
        $instance7 = $DB->get_record('enrol', array('courseid' => $course6->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance7->customint6 = 1;
        $instance7->enrolstartdate = time() + 60;
        $DB->update_record('enrol', $instance7);
        $linkedgroupsplugin->update_status($instance7, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in past.
        $instance8 = $DB->get_record('enrol', array('courseid' => $course7->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance8->customint6 = 1;
        $instance8->enrolstartdate = time() - 60;
        $DB->update_record('enrol', $instance8);
        $linkedgroupsplugin->update_status($instance8, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in future.
        $instance9 = $DB->get_record('enrol', array('courseid' => $course8->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance9->customint6 = 1;
        $instance9->enrolenddate = time() + 60;
        $DB->update_record('enrol', $instance9);
        $linkedgroupsplugin->update_status($instance9, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in past.
        $instance10 = $DB->get_record('enrol', array('courseid' => $course9->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance10->customint6 = 1;
        $instance10->enrolenddate = time() - 60;
        $DB->update_record('enrol', $instance10);
        $linkedgroupsplugin->update_status($instance10, ENROL_INSTANCE_ENABLED);

        // Maximum enrolments reached.
        $instance11 = $DB->get_record('enrol', array('courseid' => $course10->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance11->customint6 = 1;
        $instance11->customint3 = 1;
        $DB->update_record('enrol', $instance11);
        $linkedgroupsplugin->update_status($instance11, ENROL_INSTANCE_ENABLED);
        $linkedgroupsplugin->enrol_user($instance11, $user2->id, $studentrole->id);

        // Maximum enrolments not reached.
        $instance12 = $DB->get_record('enrol', array('courseid' => $course11->id, 'enrol' => 'linkedgroups'), '*', MUST_EXIST);
        $instance12->customint6 = 1;
        $instance12->customint3 = 1;
        $DB->update_record('enrol', $instance12);
        $linkedgroupsplugin->update_status($instance12, ENROL_INSTANCE_ENABLED);

        $this->setUser($user1);
        $this->assertTrue($linkedgroupsplugin->show_enrolme_link($instance1));
        $this->assertFalse($linkedgroupsplugin->show_enrolme_link($instance2));
        $this->assertFalse($linkedgroupsplugin->show_enrolme_link($instance3));
        $this->assertFalse($linkedgroupsplugin->show_enrolme_link($instance4));
        $this->assertFalse($linkedgroupsplugin->show_enrolme_link($instance7));
        $this->assertTrue($linkedgroupsplugin->show_enrolme_link($instance8));
        $this->assertTrue($linkedgroupsplugin->show_enrolme_link($instance9));
        $this->assertFalse($linkedgroupsplugin->show_enrolme_link($instance10));
        $this->assertFalse($linkedgroupsplugin->show_enrolme_link($instance11));
        $this->assertTrue($linkedgroupsplugin->show_enrolme_link($instance12));

        require_once("$CFG->dirroot/cohort/lib.php");
        cohort_add_member($cohort1->id, $user1->id);

        $this->assertTrue($linkedgroupsplugin->show_enrolme_link($instance5));
        $this->assertFalse($linkedgroupsplugin->show_enrolme_link($instance6));
    }
    ...
    public function test_can_self_enrol() {
        ...
        $DB->update_record('enrol', $instance1);
        $linkedgroupsplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);
        $linkedgroupsplugin->enrol_user($instance1, $user2->id, $editingteacherrole->id);

        $this->setUser($guest);
        $this->assertStringContainsString(get_string('noguestaccess', 'enrol'),
                $linkedgroupsplugin->can_self_enrol($instance1, true));

        $this->setUser($user1);
        $this->assertTrue($linkedgroupsplugin->can_self_enrol($instance1, true));

        // Active enroled user.
        $this->setUser($user2);
        $linkedgroupsplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $this->setUser($user1);
        $this->assertSame($expectederrorstring, $linkedgroupsplugin->can_self_enrol($instance1, true));
    }
    ...
    public function test_is_self_enrol_available() {
        ...
        // New enrolments are allowed and enrolment instance is enabled.
        $instance = $DB->get_record('enrol', ['courseid' => $course->id, 'enrol' => 'linkedgroups'], '*', MUST_EXIST);
        $instance->customint6 = 1;
        $DB->update_record('enrol', $instance);
        $linkedgroupsplugin->update_status($instance, ENROL_INSTANCE_ENABLED);
        $this->setUser($user1);
        $this->assertTrue($linkedgroupsplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertTrue($linkedgroupsplugin->is_self_enrol_available($instance));
        ...
        $instance->customint6 = 1;
        $DB->update_record('enrol', $instance);
        $linkedgroupsplugin->update_status($instance, ENROL_INSTANCE_DISABLED);
        $this->setUser($user1);
        $this->assertEquals($canntenrolerror, $linkedgroupsplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertEquals($canntenrolerror, $linkedgroupsplugin->is_self_enrol_available($instance));
        ...
        $this->setUser($user1);
        $this->assertEquals($error, $linkedgroupsplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertEquals($error, $linkedgroupsplugin->is_self_enrol_available($instance));
        ...
        $DB->update_record('enrol', $instance);
        $linkedgroupsplugin->enrol_user($instance, $user2->id, $studentrole->id);
    ...
    public function test_get_welcome_email_contact() {
        ...
        $linkedgroupsplugin = enrol_get_plugin('linkedgroups');
        $instance1 = $DB->get_record('enrol', ['courseid' => $course1->id, 'enrol' => 'linkedgroups'], '*', MUST_EXIST);
        $instance1->customint6 = 1;
        ...
        $contact = $linkedgroupsplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_COURSE_CONTACT, $context);
        ...
        $linkedgroupsplugin->enrol_user($instance1, $user1->id, $editingteacherrole->id);
        ...
        $DB->update_record('enrol', $instance1);
        $linkedgroupsplugin->enrol_user($instance1, $user3->id, $managerrole->id);

        // Give manager role holdkey capability.
        assign_capability('enrol/linkedgroups:holdkey', CAP_ALLOW, $managerrole->id, $context);

        // We should get the manager email contact.
        $contact = $linkedgroupsplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_KEY_HOLDER, $context);
        ...
        $linkedgroupsplugin->enrol_user($instance1, $user4->id, $managerrole->id);
        $contact = $linkedgroupsplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_KEY_HOLDER, $context);
        $this->assertEquals($user3->username, $contact->username);
        $this->assertEquals($user3->email, $contact->email);
        ...
        $contact = $linkedgroupsplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_NOREPLY, $context);
        $this->assertEquals($noreplyuser, $contact);
    }
