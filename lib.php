...
/**
 * Linked courses enrolment plugin.
 *
 * @package    enrol_linkedgroups
 * @copyright  2023 Justus Dieckmann WWU
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_linkedgroups_plugin extends enrol_plugin {
    ...
    public function can_edit_instance($instance) {
        ...
        return parent::can_edit_instance($instance);
    }
    ...
    public function get_action_icons(stdClass $instance) {
        if (!$this->can_edit_instance($instance)) {
            return [];
        }
        return parent::get_action_icons($instance);
    }
    ...
    public function can_self_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if (!has_capability('enrol/linkedgroups:enrolself', context_course::instance($instance->courseid))) {
            return get_string('canntenrol', 'enrol_self');
        }
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        if (!enrol_is_enabled('linkedgroups')) {
            $trace->finished();
            return 2;
        }
     ...
     * as defined by sort_by_roleassignment_authority() having 'enrol/linkedgroups:manage'
     * capability.
     *
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/linkedgroups:config', $context);
    }
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/linkedgroups:config', $context)) {
            return false;
        }
    ...
    public function add_instance($course, array $fields = null) {
        ...
        foreach ($linkedcourseids as $linkedcourseid) {
            $fieldscopy = $fields;
            $fieldscopy['customint7'] = $id;
            $linkedcourse = get_course($linkedcourseid);
            parent::add_instance($linkedcourse, $fieldscopy);
        }
        return $id;
    }
    ...
    public function update_instance($instance, $data) {
        ...
        $linkedcourseids = $data->linkedcourses;
        unset($data->linkedcourses);
        $remainingcourseids = [];

        foreach ($this->get_linked_instances($instance) as $linkedinstance) {
            if (!in_array($linkedinstance->courseid, $linkedcourseids)) {
        ...
        foreach ($linkedcourseids as $linkedcourseid) {
            if (!in_array($linkedcourseid, $remainingcourseids)) {
                ...
                $linkedcourse = get_course($linkedcourseid);
                parent::add_instance($linkedcourse, $datacopy);
            }
        }
    ...
    public function delete_instance($instance) {
        foreach ($this->get_linked_instances($instance) as $linkedinstance) {
            parent::delete_instance($linkedinstance);
        }
        parent::delete_instance($instance);
    }

    private function get_linked_instances($instance) {
        global $DB;
        return $DB->get_records('enrol', ['enrol' => 'linkedgroups', 'customint7' => $instance->id]);
    }
    ...
    public function get_welcome_email_contact($sendoption, $context) {
        ...
        if ($sendoption == ENROL_SEND_EMAIL_FROM_COURSE_CONTACT) {
            ...
            // Send as the first user with enrol/linkedgroups:holdkey capability assigned in the course.
            list($sort) = users_order_by_sql('u');
            $keyholders = get_users_by_capability($context, 'enrol/linkedgroups:holdkey', 'u.*', $sort);
            if (!empty($keyholders)) {
